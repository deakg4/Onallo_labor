...
figure(1)
surf_frac_prod = surf(X, Y, E_frac_plot_inv);
ax1 = gca
title(ax1, 'Frac inverz surface')

figure(2)
surf_frac_mean = surf(X, Y, E_frac_plot);
title('Frac surface')
...
f_min = fminsearch(E_frac_np, [2 0.1])
e_min = E_frac_np(fmin)
f_max = fminsearch(E_frac_np_inv, [2 0.1])
e_max = E_frac_np_inv(f_max)

U_optimum_1 = elmozdulasszamitas_optimum(m, f_min(1), f_min(2), force, force_pos, Ms, omegakezdo, Nomega, Kiertekeles, visszacsatolt,3);
U_optimum_2 = elmozdulasszamitas_optimum(m, f_max(1), f_max(2), force, force_pos, Ms, omegakezdo, Nomega, Kiertekeles, visszacsatolt,3);
